...
PBR 材质数据集加载器
====================
从 dataset/ 目录加载材质，每个材质包含三张 PNG：
  - *_arm_2k.png  → R: AO, G: Roughness, B: Metalness
  - *_diff_2k.png → RGB: Albedo (sRGB 编码, 解码到线性空间)
  - *_nor_dx_2k.png → RGB: Normal (DirectX, 范围 [0,1])

输出 9 通道参考张量 (全部范围 [0, 1]):
...
def load_material(material_dir, target_res=1024, output_channels='full', srgb_decode=True):
    """从材质目录加载并组装参考张量。
    ...
    Args:
        ...
        output_channels:   'full' → 9ch (albedo+normal+ao+roughness+metalness)
                           'compact' → 6ch (albedo+normal+ao)
        srgb_decode:       是否将 sRGB 编码的 PNG 解码到线性空间 (默认 True)

    Returns:
        ref_tensor: [C, H, W] float32 张量，范围 [0, 1]
    """
    ...
    # --- ARM: R=AO, G=Roughness, B=Metalness (可选 sRGB → Linear) ---
    arm = np.array(Image.open(arm_path).convert('RGB'), dtype=np.float32) / 255.0
    if srgb_decode:
        arm = np.power(arm, 2.2)
    ao = arm[..., 0:1]         # R → AO
...
class MaterialDataset(Dataset):
    ...
    def __init__(self, root_dir, target_res=1024, preload=True, output_channels='full', srgb_decode=True):
        self.root_dir = root_dir
        self.target_res = target_res
        self.output_channels = output_channels
        self.srgb_decode = srgb_decode
    ...
    def _load_one(self, name):
        ...
        ref = load_material(dpath, self.target_res, self.output_channels, self.srgb_decode)
